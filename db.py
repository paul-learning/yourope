...
def ensure_schema(conn: sqlite3.Connection) -> None:
    ...
    # Legacy: Round actions (generated by GM, then published)
    cur.execute("""
    ...
    # Legacy: Locks (players lock choice)
    cur.execute("""
    ...
    # Domestic events per round + country
    cur.execute("""
    ...
    # --- migrations: quote + craziness for external events ---
    ...
    # Country snapshots for public dashboard
    cur.execute("""
    ...
    # -------------------------
    # NEW: Player-generated policy candidates (up to 3 per domain)
    # -------------------------
    cur.execute("""
    CREATE TABLE IF NOT EXISTS policy_candidates (
        round INTEGER NOT NULL,
        country TEXT NOT NULL,
        domain TEXT NOT NULL,            -- "foreign" | "domestic"
        slot INTEGER NOT NULL,           -- 1..3
        aggressiveness INTEGER NOT NULL, -- 0..100
        action_text TEXT NOT NULL,
        impact_json TEXT NOT NULL DEFAULT '',
        ts DATETIME DEFAULT CURRENT_TIMESTAMP,
        PRIMARY KEY (round, country, domain, slot)
    )
    """)

    # -------------------------
    # NEW: Locks for chosen slots per domain
    # -------------------------
    cur.execute("""
    CREATE TABLE IF NOT EXISTS policy_locks (
        round INTEGER NOT NULL,
        country TEXT NOT NULL,
        locked_foreign_slot INTEGER,
        locked_domestic_slot INTEGER,
        locked_at DATETIME DEFAULT CURRENT_TIMESTAMP,
        PRIMARY KEY (round, country)
    )
    """)
    ...
    if cur.fetchone() is None:
        # phases: setup -> external_generated -> actions_published -> game_over
...
def clear_round_data(conn: sqlite3.Connection, round_no: int) -> None:
    ...
    # new tables
    cur.execute("DELETE FROM policy_candidates WHERE round = ?", (int(round_no),))
    cur.execute("DELETE FROM policy_locks WHERE round = ?", (int(round_no),))
    conn.commit()
...
# -----------------------
# NEW: Policy candidates + locks (3 tries per domain)
# -----------------------
def upsert_policy_candidate(
    conn: sqlite3.Connection,
    *,
    round_no: int,
    country: str,
    ...
    if domain not in ("foreign", "domestic"):
        raise ValueError("domain must be 'foreign' or 'domestic'")
    slot_i = int(slot)
    ...
    cur = conn.cursor()
    cur.execute("""
        INSERT INTO policy_candidates (round, country, domain, slot, aggressiveness, action_text, impact_json)
        VALUES (?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT(round, country, domain, slot) DO UPDATE SET
    ...
    """, (
        ...
        str(domain),
        int(slot_i),
        clamp_int(int(aggressiveness), 0, 100),
        str(action_text),
        str(impact_json),
    ))
    conn.commit()
...
def get_policy_candidates(
    ...
    *,
    ...
    cur = conn.cursor()
    cur.execute("""
        SELECT slot, aggressiveness, action_text, impact_json, ts
        FROM policy_candidates
        WHERE round = ? AND country = ? AND domain = ?
        ORDER BY slot ASC
    """, (int(round_no), str(country), str(domain)))

    out: List[Dict[str, Any]] = []
    for slot, aggressiveness, action_text, impact_json, ts in cur.fetchall():
        impact = {}
        try:
            impact = json.loads(impact_json or "{}")
        except Exception:
            impact = {}
        out.append({
            "slot": int(slot),
...
def lock_policy_slot(
    ...
    *,
    ...
    cur = conn.cursor()
    cur.execute("INSERT OR IGNORE INTO policy_locks (round, country) VALUES (?, ?)", (int(round_no), str(country)))

    if domain == "foreign":
        cur.execute("""
            UPDATE policy_locks
            SET locked_foreign_slot = ?, locked_at = CURRENT_TIMESTAMP
            WHERE round = ? AND country = ?
        """, (slot_i, int(round_no), str(country)))
    else:
        cur.execute("""
            UPDATE policy_locks
            SET locked_domestic_slot = ?, locked_at = CURRENT_TIMESTAMP
            WHERE round = ? AND country = ?
        """, (slot_i, int(round_no), str(country)))
...
def get_policy_locks(conn: sqlite3.Connection, *, round_no: int) -> Dict[str, Dict[str, Optional[int]]]:
    cur = conn.cursor()
    cur.execute("""
        SELECT country, locked_foreign_slot, locked_domestic_slot
        FROM policy_locks
        WHERE round = ?
    """, (int(round_no),))
    out: Dict[str, Dict[str, Optional[int]]] = {}
    for country, f, d in cur.fetchall():
        out[str(country)] = {
            "foreign": (int(f) if f is not None else None),
            "domestic": (int(d) if d is not None else None),
        }
    return out


def all_policies_locked(conn: sqlite3.Connection, *, round_no: int, countries: List[str]) -> bool:
    locks = get_policy_locks(conn, round_no=round_no)
    for c in countries:
        v = locks.get(c) or {}
        if not v.get("foreign") or not v.get("domestic"):
            return False
    return True
...
def clear_all_events_and_history(conn: sqlite3.Connection) -> None:
    ...
    cur.execute("DELETE FROM policy_candidates")
    cur.execute("DELETE FROM policy_locks")
    conn.commit()
